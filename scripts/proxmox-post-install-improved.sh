...
# Ceph bridge (for VM access to storage network)
auto vmbr1
...
EOF
    ...
    log "Configured Ceph network on eno3 with MTU 9000"
else
...
# Send public key to provisioning server
PUBLIC_KEY=$(cat /root/.ssh/id_ed25519.pub)
curl -X POST \
    -H "Content-Type: application/json" \
    -d "{\"hostname\":\"$HOSTNAME\",\"ip\":\"$IP_ADDRESS\",\"public_key\":\"$PUBLIC_KEY\",\"node_type\":\"proxmox\"}" \
    "http://$PROVISION_SERVER/api/collect-ssh-key.php" \
    || log "Warning: Failed to send SSH key to provisioning server"

# Retrieve all SSH keys (after brief delay for processing)
sleep 3
...
if [ -n "$KEYS_RESPONSE" ] && echo "$KEYS_RESPONSE" | jq -e .success >/dev/null 2>&1; then
    ...
    log "Updated authorized_keys with cluster SSH keys"
fi
...
if [ "$HOSTNAME" == "$CLUSTER_PRIMARY" ]; then
    ...
    if ! pvecm status >/dev/null 2>&1; then
        ...
        if pvecm status | grep -q "Quorum provider"; then
            log "✓ Cluster is operational and has quorum"
        else
    ...
    if pvecm status >/dev/null 2>&1; then
        ...
        if ping -c 1 -W 2 "$CLUSTER_PRIMARY_IP" >/dev/null 2>&1; then
            log "✓ Can reach primary node ($CLUSTER_PRIMARY_IP)"
        else
            log "✗ Cannot reach primary node ($CLUSTER_PRIMARY_IP)"
        fi
        
        # Test Ceph network connectivity
        if ping -c 1 -W 2 "${CEPH_IPS[$CLUSTER_PRIMARY]}" >/dev/null 2>&1; then
            log "✓ Can reach primary node Ceph network (${CEPH_IPS[$CLUSTER_PRIMARY]})"
        else
            log "✗ Cannot reach primary node Ceph network"
        fi
        
        # Test SSH connectivity (will fail initially until keys are exchanged)
        if ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no root@"$CLUSTER_PRIMARY_IP" echo "SSH-OK" >/dev/null 2>&1; then
            log "✓ SSH connectivity to primary node works"
        else
            log "⚠ SSH connectivity to primary node not ready (keys may need exchange)"
        fi
